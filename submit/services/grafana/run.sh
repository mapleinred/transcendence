...
# Retrieving grafana password from Vault server
GRAFANA_TOKEN=$(cat /run/secrets/grafana-token)
ADMIN_PASSWORD=$(curl --cacert /etc/ssl/certs/ca.crt -H "X-Vault-Token: $GRAFANA_TOKEN" $VAULT_ADDR/v1/kv/grafana-creds \
	| jq . | grep "grafana-password" | awk '{print $2}' | tr -d '"')
# grafana requires this var to be set for its admin passwordd
...
# Remember to put the sole '&' sign at the very back before any redirections, else it would not properly redirect
/init.sh > /output.txt 2>&1 &
exec grafana server                                         \
