...
# Force Elasticsearch to retrieve ELASTIC_PASSWORD from Vault instead of regular env var
ELASTIC_TOKEN=$(cat /run/secrets/elastic-token)
# Format of curl output differs when writing to terminal vs file (eg pipe), in this case
# the JSON object it returns is squeezed into a single line. Using jq helps to format into
# multi-line JSON object 
ELASTIC_PASSWORD=$(curl --cacert /usr/share/elasticsearch/config/certs/ca/ca.crt -H "X-Vault-Token: $ELASTIC_TOKEN" $VAULT_ADDR/v1/kv/elastic-creds \
	| jq . | grep "elastic-password" | tr -d '"' | awk '{print $2}')
